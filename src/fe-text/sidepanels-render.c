                            ...
                            const char *text)
{
    ...
    if (!tw || !wctx) {
        sp_logf("ERROR: draw_str_themed called with NULL tw=%p or wctx=%p", 
                (void *) tw, (void *) wctx);
        return;
    }
                                    ...
                                    const char *param1, const char *param2)
{
    ...
    if (!tw || !wctx) {
        sp_logf("ERROR: draw_str_themed_2params called with NULL tw=%p or wctx=%p",
                (void *) tw, (void *) wctx);
        return;
    }
...
void redraw_both_panels_only(const char *event_name)
{
    ...
    for (t = mainwindows; t; t = t->next) {
        ...
        /* Position panels once for both sides */
        if ((ctx->left_tw && ctx->left_h > 0) || (ctx->right_tw && ctx->right_h > 0)) {
            position_tw(mw, ctx);
        }
...
{
    const char *event_name = data ? (const char *) data : "batched_redraw";
...
void schedule_batched_redraw(const char *event_name)
{
    ...
    event_copy = event_name ? g_strdup(event_name) : NULL;

    redraw_pending = TRUE;
    batch_mode_active = TRUE;
    redraw_timer_tag = g_timeout_add_full(G_PRIORITY_DEFAULT,
                                         redraw_batch_timeout,
                                         batched_redraw_timeout,
                                         event_copy,
                                         g_free);

    if (redraw_timer_tag == 0) {
        /* Fallback: scheduling failed, clean up state */
        redraw_pending = FALSE;
        batch_mode_active = FALSE;
        g_free(event_copy);
    }
